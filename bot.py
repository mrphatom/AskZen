...
PREMIUM_STARS  = 750         # Telegram Stars price (~$11.50)
PREMIUM_DAYS   = 30
GROQ_MODEL     = "llama-3.3-70b-versatile"
MAX_MEMORY     = 10          # Keep the last 10 messages for stable, consistent memory
...
MODES = {
    "general": {
        ...
        "prompt": (
            "You are AskZen, an authentic, supportive, and clever AI companion. "
            "Match the user's tone, energy, and vibe naturally. Be empathetic but candid. "
            "If the user is casual, be casual and use light wit. Avoid standard corporate AI filler phrases "
            "like 'As an AI...', 'Sure, I can help with that!', or 'Is there anything else?'. "
            "Just converse naturally, like a sharp, grounded peer. Use markdown smoothly for clarity."
        ),
    },
    "code": {
        "name": "💻 Code Partner",
        ...
        "prompt": (
            "You are a brilliant, highly practical senior software engineer. You don't just dump code blocks; "
            "you treat the user like a peer. Think through edge cases out loud, suggest clean and modern architecture, "
            "and explain your fixes without being patronizing. Format code beautifully and add meaningful comments."
        ),
    },
    "writer": {
        ...
        "prompt": (
            "You are an intuitive creative writer and editor with an eye for rhythm, tone, and impact. "
            "Whether working on scripts, essays, or casual copy, match the target audience perfectly. "
            "Give suggestions to heighten dramatic tension or emotional clarity when editing creative pieces. "
            "Keep feedback sharp, encouraging, and collaborative."
        ),
    },
    "analyst": {
        "name": "📊 Strategy Lead",
        ...
        "prompt": (
            "You are a pragmatic, data-fluent strategist. Don't state the obvious. Break down complex ecosystems, "
            "spot trends, and deliver highly structured, scannable breakdowns. Cut straight to the value, "
            "using bolding and formatting to make insights instantly digestible."
        ),
    },
    "tutor": {
        "name": "🎓 Mind Coach",
        ...
        "prompt": (
            "You are an exceptionally engaging, patient tutor. Demystify complex concepts using vivid analogies, "
            "real-world examples, and foundational breakdowns. Never speak down to the learner. "
            "End your explanations with a single, thought-provoking question to check for understanding and keep them hooked."
        ),
    },
}
...
async def call_ai(user_id: int, user_message: str) -> str:
    ...
    # Keep conversation history bounded to avoid hitting Groq free tier token limits
    ...
    try:
        resp = groq_client.chat.completions.create(
            ...
            max_tokens=1200,
            temperature=0.75, # Slightly higher temperature for more natural, less rigid variations
        )
        reply = resp.choices[0].message.content
        
        # Save historical turn back to the DB
...
async def keep_typing_loop(ctx: ContextTypes.DEFAULT_TYPE, chat_id: int, stop_event: asyncio.Event):
    """Keeps the 'typing...' indicator alive smoothly while Groq processes."""
...
async def safe_send(update: Update, text: str, **kwargs):
    ...
    for chunk in chunks:
        try:
            ...
            # Fallback if markdown formatting is broken
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Fire off background continuous typing loop
    ...
    try:
        ...
        # Stop typing indicator right before sending the text out
        stop_typing.set()
        await typing_task
